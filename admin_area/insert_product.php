...
<select name="sell_percent" class="form-control">
	...
	while ($row=mysqli_fetch_array($run_p_label)) {
		...
		echo "<option value='$s_id'> $sell_per </option>";
	}
	?>
</select>
</div>
...
<div class="form-group">
<input type="submit" name="submit" value="Insert Product" class="bat btn-primary form-control">
</div>
...
if (isset($_POST['submit'])) {
	...
	$insert_product="insert into product(p_cat_id,cat_id,date,product_title,product_img1,product_img2,product_img3,product_price,product_desc,product_keyword,product_label_id,sell_id) values('$product_cat','$cat',NOW(),'$product_title','$product_img1','$product_img2','$product_img3','$product_price','$product_desc','$product_keyword','$product_label','$product_sell')";

	$run_product=mysqli_query($con,$insert_product);
	if ($run_product) {
		echo "<script> alert('Product Inserted Sucessfully')</script>";
		echo " <script> window.open('index_admin.php?view_product')</script>";
	}
}

...
<div class="box">
	...
	<div class="text-center">
		<button class="btn btn-primary btn-lg" name="updat" type="submit">
			Update Now
		</button>
	</div>
</form>
...
if (isset($_POST['updat'])) {
	...
	if ($check_old_pass==0) {
		echo "<script>alert('your enter password is not valid... Try again')</script>";
		exit();
	}if ($new_password!=$c_n_password) {
		echo "<script>alert('your enter password dose not match with confirm password... Try again')</script>";
		exit();
	}
	$update_q="update customers set customer_pass='$new_password' where customer_email='$c_email'";
	$run_q=mysqli_query($con,$update_q);
	echo "<script>alert('your password has been changed......Login again')</cript>";
